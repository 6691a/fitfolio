...
- 예외: 서비스는 도메인 예외(`UnsupportedDocumentFormatError`·`FileTooLargeError`·`DocumentFileParseError`·`InsufficientJobContentError`)를 던지고 `HTTPException`은 만들지 않는다. HTTP 매핑은 `app/main.py` 전역 핸들러(`_DOMAIN_EXCEPTION_STATUS`)에 모은다(새 예외는 여기 등록). 컨트롤러는 입력 검증·404에만 `HTTPException` 사용.
