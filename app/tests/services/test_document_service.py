...
async def test_request_application_parse_dispatches_resume_and_url_job_concurrently():
    ...
    class Svc(DocumentService):
        ...
        async def request_parse(self, *, document_type, format, file_name, file_path, content_type, user_id) -> int:
            resume_started.set()
            await job_started.wait()
            return 11

        async def request_parse_url(self, *, document_type, url, user_id) -> int:
            job_started.set()
